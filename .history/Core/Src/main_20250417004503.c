...
int main(void)
{
  ...
  // 电机驱动电路（集成驱动芯片DRV8313）    根据 PWM 信号驱动 逆变电路，控制电机绕组电流  PWM 输入引脚（IN1/IN2/IN3）与 MCU 的定时器通道（如 TIM1_CH1~CH3）相连
  ...
  HAL_TIM_Base_Start_IT(&htim3);            // 对应tim.c中的HAL_TIM_PeriodElapsedCallback回调         //这行代码会使能 TIM3 的 更新事件中断（TIM_IT_UPDATE），当 TIM3 的计数器达到自动重装载值（Period）时，触发中断。
  // ADC电流采样
  /*
  校准目的
